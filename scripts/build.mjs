...
const lessons=list.map((l,i)=>{
 ...
 return {...l,index:i+1,en:entry.title,source:entry.url,refs:l.refs.map(key=>{if(!sources[key])throw Error('Unknown source '+key);return sources[key];})};
});
...
const payload={version:1,date:catalog.date,groups,lessons,learningOrder};
